...
export default function getSchema(provider) {
  ...
  if (features.backchannelLogout.enabled) {
    RECOGNIZED_METADATA.push('backchannel_logout_session_required', 'backchannel_logout_uri');
  }

  if (features.requestObjects.enabled) {
    RECOGNIZED_METADATA.push('request_object_signing_alg', 'require_signed_request_object');
    if (features.encryption.enabled) {
      RECOGNIZED_METADATA.push('request_object_encryption_alg', 'request_object_encryption_enc');
    }
  }
  ...
  if (features.encryption.enabled) {
    RECOGNIZED_METADATA.push('id_token_encrypted_response_alg', 'id_token_encrypted_response_enc');
    if (features.jwtUserinfo.enabled) {
      RECOGNIZED_METADATA.push(
  ...
  if (features.rpMetadataChoices.enabled) {
    CHOICES = Object.fromEntries(
      Object.entries(CLIENT_ATTRIBUTES.CHOICES).filter(([key]) => RECOGNIZED_METADATA.includes(key))
    );
  ...
  const ENUM = {
    ...
    authorization_details_types: () => Object.keys(features.richAuthorizationRequests.types),
    ...
    #initialize(metadata) {
      Object.assign(
        ...
        omitBy(
          ...
          (value, key) =>
            isUndefined(value) || (key in CHOICES && metadata[CHOICES[key]] !== undefined)
        ),
        omitBy(
      ...
      if (
        this.grant_types.some((type) => ['authorization_code', 'implicit'].includes(type)) &&
